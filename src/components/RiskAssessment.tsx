...
export const RiskAssessment = ({
  ...
  const getUserGeolocation = (): string => {
    ...
    return mockGeolocations[
      Math.floor(Math.random() * mockGeolocations.length)
    ];
  };
  ...
  const analyzeDocumentQuality = (
    ...
    const qualities = {
      ...
      Other: {
        quality: 40,
        issues: ["Unsupported document type", "Poor quality"],
      },
    };

    return (
      qualities[documentType as keyof typeof qualities] || qualities["Other"]
    );
  };

  // Detect behavioral anomalies
  const detectBehaviorAnomalies = (
    ...
    const avgTimePerField =
      metrics.timeBetweenFields.reduce((a, b) => a + b, 0) /
      metrics.timeBetweenFields.length;
    if (avgTimePerField < 500) {
      // Less than 0.5 seconds per field
    ...
    if (metrics.sessionDuration < 30000) {
  ...
  const calculateRiskScore = (
    ...
    if (userGeoLocation !== data.country) {
      score += 20;
      factors.push(
        `Geolocation mismatch: IP from ${userGeoLocation}, declared country ${data.country}`
      );
    }
    ...
    if (documentAnalysis.issues.length > 0) {
      factors.push(
        ...documentAnalysis.issues.map((issue) => `Document issue: ${issue}`)
      );
    }
      ...
      <div className="max-w-2xl mx-auto">
        <Card className="border-0 shadow-2xl bg-white/90 backdrop-blur-sm">
          <CardHeader className="text-center pb-8">
            ...
            <CardDescription>
              Analyzing profile, behavior, and document readiness
            </CardDescription>
          </CardHeader>
    ...
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-8">
        ...
        <p className="text-lg text-gray-600">
          Our AI analyzes your profile, behavior, and document readiness to
          customize your verification experience
        </p>
      </div>
      ...
      <Card className="border-0 shadow-2xl bg-white/90 backdrop-blur-sm">
        <CardHeader>
          ...
          <CardDescription>
            We analyze multiple factors including location, behavior patterns,
            and document quality indicators
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
